...
def create_text_chunks(documents):
    try:
        ...
        logger.info(f"Splitting {len(documents)} documents into text chunks with chunk size {CHUNK_SIZE} and overlap {CHUNK_OVERLAP}")

        text_splitter = RecursiveCharacterTextSplitter(chunk_size=CHUNK_SIZE, chunk_overlap=CHUNK_OVERLAP)

        text_chunks = text_splitter.split_documents(documents)

        logger.info(f"Created {len(text_chunks)} text chunks from {len(documents)} documents")
        return text_chunks
    
    except Exception as e:
        logger.error(f"Error creating text chunks: {e}")
        raise CustomException("Error creating text chunks", e)